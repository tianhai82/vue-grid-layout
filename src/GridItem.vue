<template>
<div ref="item" class="vue-grid-item" :class="{ 'vue-resizable' : isResizable, 'resizing' : isResizing, 'vue-draggable-dragging' : isDragging, 'cssTransforms' : useCssTransforms }"
    :style="style">
    <slot></slot>
...
export default {
    ...
    computed: {
        width: function() {
            ...
            return this.w === Infinity ? this.w : Math.round(colWidth * this.w + Math.max(0,
                this.w - 1) * this.margin[0]);
        },
        height: function() {
            const colWidth = this.calcColWidth();
            return this.h === Infinity ? this.h : Math.round(this.rowHeight * this.h + Math.max(
                0, this.h - 1) * this.margin[1]);
        },
    },
    watch: {
        cols: function() {
    ...
    methods: {
        ...
        calcPosition: function(x, y, w, h) {
            ...
            const out = {
                ...
                width: w === Infinity ? w : Math.round(colWidth * w + Math.max(0, w - 1) *
                    this.margin[0]),
                height: h === Infinity ? h : Math.round(this.rowHeight * h + Math.max(0, h -
                    1) * this.margin[1])
            };
        ...
        notifyResize: function() {
            this.$emit('resized', {
                i: this.i,
                ...this.calcPosition(this.x, this.y, this.w, this.h)
            })
        },
    },
}
